...
description: >-
  ...
  pick a side: a tooling-doctrine question, a design that returned two defensible shapes,
...
1. A **Tier-3 or doctrine fork** — a decision that changes what the SDK means, is
   expensive to reverse, or contradicts an existing invariant (`docs/internal/invariants.md`)
   or a closed decision in the main repository's architecture decision record.
...
the invariants and the vendored standards text, and cites `file:line`. A panelist that
reasons from the brief alone is producing an opinion, and opinions are what this
...
1. **Mark each claim** verified, refuted, or unverifiable — against the live code, the
   invariants and the vendored standards, not against plausibility. Unverifiable claims
   are **discarded**, not weighed.
...
3. **DEFER to the owner** when the rule does not discriminate, and say precisely what
   additional evidence would — including, where relevant, that the question is owned
   main-side (standards semantics, gateway behavior) and needs a corpus owner.
...
rather than on memory. Where the outcome changes tooling doctrine, it becomes a
decision-record entry when it lands.
