# Provenance: adopted 2026-09-15 from the main benchweave-ui repository's memory system (hooks + protocol), which adopted it 2026-09-10 from the muninndb repository. Its measured history (declaration rates, transcript counts, incident numbers) is muninndb's, kept for the mechanism's rationale. This repo drains to the same `benchweave` vault and adds one rule of its own: **every proposal from this repository carries the `sdk` tag.**
...
*remembering to remember* does not happen under load — the muninndb project measured that
...
The bar exists because **a noisy vault is worse than a small one.** That is measured on
muninndb too: a pollution analysis found 74% of supersession pairs were template junk, and
it collapsed the analysis that depended on them. A drain with no bar is a pollution pump.
...
{"concept":"short label","content":"the fact itself, self-contained, readable in a year","summary":"one line","type":"fact","tags":["sdk","…"],"source":"adversary"}
JSON
```
...
because prose did not enforce it in muninndb: 43 of the first 179 proposals were permanently
...
rule as committed content: no credentials, no client identifiers, no memory content copied
out of someone else's vault. It is gitignored so it cannot land by accident — that is a
backstop, not permission.

## How it drains

It drains itself. `PreCompact`, `SessionEnd` and a debounced `Stop` are wired in
`.claude/settings.json`; the measurement behind that choice (muninndb lineage, over 55
...
It does not recall the neighbourhood before each write. muninndb's first version did, and
**HOLD**ing anything with a strong-banded match was retracted after four independent
reviews: the gate never executed once in its lifetime, and four defects lived in it —
...
- HELD was unbounded and grew *because* the drain succeeded, since every write densifies
  the neighbourhood the next proposal is tested against.
...
So it stays a separate, explicitly human-run pass that reads the **vault**, not the
ledger — near-duplicate clusters among recently-written engrams, adjudicated with
`muninn_evolve`. **That pass is not built yet** (it is not built on the main repo either).
Until it is, the vault can accumulate near-duplicates from the drain, and that is the
accepted cost of a durability path that actually runs. Nothing about the pipe blocks it:
`memory-proposals.drained.jsonl` records every engram id the drain produced, which is
exactly the candidate set such a pass needs.

### Worktrees — one ledger per repository

**There is exactly one ledger per repository: this repository's main checkout's.** A linked
worktree's `.git` is a file pointing at `<main>/.git/worktrees/<name>`, and `paths()`
resolves through it, so a proposal appended with a worktree as the working directory queues
in the main checkout where a drain will actually look. `MUNINN_LEDGER_ROOT` overrides it.

This corrects a claim that was false when it was written (muninndb lineage). The previous
version said each worktree "drains its own ledger… no ledger is invisible to the process
that would drain it". It was invisible: `CLAUDE_PROJECT_DIR` is **empty in a Bash tool
environment**, so `paths()` fell back to `cwd`, and an append made from a scratch worktree
landed there — but no session ever has a scratch worktree as its project root, so nothing
ever drained it and `ledger-guard.mjs` never saw it either. 17 real proposals were found
stranded across two worktrees, silently, while the document called the arrangement correct.
A per-worktree queue only works if a drain runs per worktree, and none ever will.

One consequence specific to this repository's layout: the submodule checkout at
`packages/sdk` inside the main benchweave-ui repository is its own checkout of this repo —
`.git` points at `../../.git/modules/packages/sdk`, which has **no `commondir` file**, so
`canonicalRoot` resolves to the submodule directory itself, not the parent repo. A proposal
made with the submodule path as `cwd` queues in `packages/sdk/.claude/`, where no session is
rooted — prefer proposing from a session rooted in a real checkout of this repository
(e.g. the standalone clone), where the lifecycle hooks actually fire.
