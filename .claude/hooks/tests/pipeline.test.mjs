...
//
// NOT in CI: the CI budget is for ruff/mypy/self-check, and this tooling is developer-local.
// It is fast (a few seconds, one loopback HTTP server, no daemon) so there is no excuse for
// not running it before touching the drain.
//
// Every test that came across the port exists because the behaviour it pins was measured
// broken, and each one was shown to fail against the pre-fix drain before the fix landed
// (issue #825, muninndb/main lineage).
//
...
test('tags are required — a tag-less proposal is rejected (observed on the benchweave vault 2026-09-11)', () => {
  ...
  const okTags = validate(proposal(3, { tags: ['gotcha', 'sdk'] }))
...
test('a ledger line without the sdk tag dead-letters at the drain — there is no untagged path into the vault', async (t) => {
  const srv = await fakeMuninn()
  t.after(() => srv.close())
  const { root } = makeRepo([proposal(1, { tags: ['gotcha'] })])
  await runNode(DRAIN, ['--base', srv.base], { root })
  const rc = readReceiptFile(root)
  assert.equal(rc.counts.dead_lettered, 1)
  assert.equal(rc.counts.written, 0)
