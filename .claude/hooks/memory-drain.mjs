...
// survive load — this project measured that (a 4.81% declaration rate that three
// interventions failed to move) and then demonstrated it on itself, losing most of a day's
// findings while building the memory engine.
//
// So agents append to .claude/memory-proposals.jsonl and this drains it.
//
// ── What changed, and why (issue #825) ───────────────────────────────────────────────────
//
// The first version of this script also performed a vault recall per proposal and HELD
// anything with a strong-banded neighbour, so a script would never mint a rival copy. It
// never ran, and four independent reviews converged on the same verdict: that check is
// curation doing durability work, and putting a judgment call that requires a human on the
// critical path of a mechanism whose entire purpose is to not require one is the bug.
//
// Four defects lived in that recall loop and all four die with it:
//   - it cost ~0.9s/proposal, which is what made the drain unusable in a hook;
//   - the gate was structurally dead on any vault whose fusion mode is rrf or weighted_sum
//     (and whenever semantic search is degraded), because the engine then bands every row
//     `uncalibrated` and `strong` is never emitted — so the drain wrote everything while
//     reporting "0 held", indistinguishable from "0 duplicates found";
...
const BASE = argOf('--base') || process.env.MUNINN_MCP_URL || 'http://127.0.0.1:8750/mcp'
