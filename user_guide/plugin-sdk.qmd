...
The writer is three of the eight members. A bench script that drives an adapter without a gateway needs the other five as well: clocks, evidence, and the transport. The example below is one complete `CaptureServices` object for a device on a serial port, adapted from the serial host a contributor fork of the ADC board wrote for the same purpose. It implements the three scoped stream transactions of OTDP §8.1 exactly: `stream_send`, `stream_receive` and `stream_exchange`, with `exact_bytes` taking precedence over `termination`, `lf`/`crlf` only (serial has no `eom`), unspecified fields refused, and an incomplete frame never returned as complete. Bytes already read when a deadline expires stay buffered for the next receive, so a slow reply is not lost. The capture methods delegate to `StandaloneCaptureWriter`. This is author-side tooling, not an SDK module: copy it into your project and adapt it.
...
class BenchContext:
    ...
    def __init__(self, operation_id: str, timeout_s: float) -> None:
...
class SerialStandaloneHost:
    ...
    deadline or cancellation is noticed) and ``close()``.
    """

    def __init__(self, port: Any, *, writer: Any = None, evidence_path: Path | None = None,
                 max_frame_bytes: int = 4096) -> None:
        self._port = port
    ...
    async def transfer(self, transaction: dict[str, Any], context: Any) -> dict[str, Any]:
        ...
        if kind != "stream_receive":
            try:
                await asyncio.to_thread(self._port.write, transaction["data"])
    ...
    async def record_evidence(self, entry: dict[str, Any], context: Any) -> None:
        record = {"at": self.utc_now(), "operation_id": context.operation_id, **entry}
